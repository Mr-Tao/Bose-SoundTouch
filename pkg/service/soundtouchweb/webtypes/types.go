...
type DeviceConnection struct {
	...
	// fieldRevision orders independent REST poll results against live events.
...
func (c *DeviceConnection) ApplyPolledVolume(generation uint64, volume *models.Volume) bool {
	return c.ApplyPolledVolumeAtFieldRevision(generation, volume, c.NextFieldRevision())
}

// ApplyPolledVolumeAtFieldRevision applies a poll revision reserved before
// network I/O, preserving ordering against a newer live volume event.
func (c *DeviceConnection) ApplyPolledVolumeAtFieldRevision(
	generation uint64,
	...
	c.UpdateStatus(func(status *DeviceStatus) {
		applied = status.MergeVolume(volume, fieldRevision)
	})
...
func (c *DeviceConnection) ApplyVolumeEventChanged(volume *models.Volume, activity time.Time) bool {
	...
	connectivityChanged := c.ApplySpeakerEventChanged(func(status *DeviceStatus) {
		...
		status.MergeVolume(volume, fieldRevision)
...
func (c *DeviceConnection) ApplyPresetEvent(presets *models.Presets, activity time.Time) bool {
	...
	connectivityChanged := c.ApplySpeakerEventChanged(func(status *DeviceStatus) {
		...
		status.MergePresets(presets, fieldRevision)
		status.LastActivity = activity
	})
...
func (c *DeviceConnection) ApplyPolledBass(generation uint64, bass *models.Bass) bool {
	_, applied := c.applyPolledBassWithRevisions(generation, bass, c.NextFieldRevision())
...
func (c *DeviceConnection) ApplyPolledBassWithRevision(generation uint64, bass *models.Bass) (uint64, bool) {
	return c.applyPolledBassWithRevisions(generation, bass, c.NextFieldRevision())
}

// ApplyPolledBassAtFieldRevision applies a poll revision reserved before
// network I/O, preserving ordering against a newer live bass event.
func (c *DeviceConnection) ApplyPolledBassAtFieldRevision(
	generation uint64,
	bass *models.Bass,
	fieldRevision uint64,
) bool {
	_, applied := c.applyPolledBassWithRevisions(generation, bass, fieldRevision)

	return applied
}

func (c *DeviceConnection) applyPolledBassWithRevisions(
	generation uint64,
	bass *models.Bass,
	fieldRevision uint64,
) (uint64, bool) {
	...
	c.UpdateStatus(func(status *DeviceStatus) {
		if !status.MergeBass(bass, fieldRevision) {
			return
		}
...
func (c *DeviceConnection) ApplyBassEventChanged(bass *models.Bass, activity time.Time) bool {
	...
	connectivityChanged := c.ApplySpeakerEventChanged(func(status *DeviceStatus) {
		...
		status.MergeBass(bass, fieldRevision)
		status.BassRevision++
		status.LastActivity = activity
	})
...
func (c *DeviceConnection) CompleteHTTPPoll(
	...
	return c.completeHTTPPoll(generation, success, at, merge, false)
}

// CompleteHTTPPollFields uses field revisions to merge an otherwise-current
// poll across unrelated speaker events. Same-field events reserve newer field
// revisions and still fence older payloads.
func (c *DeviceConnection) CompleteHTTPPollFields(
	generation uint64,
	success bool,
	at time.Time,
	merge func(*DeviceStatus),
) bool {
	return c.completeHTTPPoll(generation, success, at, merge, true)
}
...
func (c *DeviceConnection) completeHTTPPoll(
	...
	c.UpdateStatus(func(status *DeviceStatus) {
		if merge != nil && knownGeneration && (fieldFenced || pollEventGeneration == c.speakerEventGen) {
			merge(status)
		}
...
// concurrent changes from other goroutines. Public and field revisions are
// derived from the current connection state rather than trusted from callers.
...
// MergeVolume applies a value unless a newer event or readback already wrote
// the field.
...
// MergeSourcesFailure records an unsuccessful source refresh while retaining
// the last successful inventory for display. The retained inventory is stale
// and cannot be used as command authority.
...
// MergeBass applies a value unless a newer event or readback already wrote the
// field.
...
// MergeIsConnected applies a value unless a newer connection projection
