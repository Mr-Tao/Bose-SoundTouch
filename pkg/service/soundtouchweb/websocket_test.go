...
func TestUpdateDeviceStatusNotConnectedWhenOnlyGroupSucceeds(t *testing.T) {
	...
	conn := webtypes.NewDeviceConnection(
		client.NewClientFromHost(server.URL),
		&models.DeviceInfo{Type: "SoundTouch 10"},
	)
	app := NewWebApp()
	app.UpdateDeviceStatus("device-1", conn)
	...
	if status.HTTPReachable {
		t.Fatal("group success alone must not count as a successful HTTP status poll")
	}
	if status.Connectivity != webtypes.ConnectivityStale {
		t.Fatalf("first failed substantive poll connectivity = %q, want stale hysteresis", status.Connectivity)
	}

	app.UpdateDeviceStatus("device-1", conn)

	status = conn.Status()
	if status.HTTPReachable || status.IsConnected || status.Connectivity != webtypes.ConnectivityOffline {
		t.Fatalf(
			"group-only success after failure threshold = HTTPReachable %t, IsConnected %t, connectivity %q; want false, false, offline",
