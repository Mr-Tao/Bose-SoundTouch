...
export function Zone({ deviceId, devices, volumePreview = null }) {
    ...
    // Candidates come from the raw physical registry so a stereo member hidden
    // by the logical device projection can still be added to an unrelated zone.
    const zoneIPs = new Set([
        zone.masterIp,
        zone.master?.ip,
        ...(zone.members || []).flatMap(member => [
            member.ip,
            ...(member.physicalMembers || []).map(physical => physical.ip),
        ]),
    ].filter(Boolean));
    const selectedHardwareId = devices?.[deviceId]?.info?.device_id;
    const available = Object.entries(candidates).filter(([ip, candidate]) =>
        !zoneIPs.has(ip) && candidate.info?.device_id !== selectedHardwareId);
