...
func TestClockDisplayOmittedEnableStateRemainsNonWritable(t *testing.T) {
	...
	if fixture.infoGets.Load() != 0 {
		t.Fatalf("live identity reads = %d, want 0 before rejected write", fixture.infoGets.Load())
	}
}
...
func TestSettingsMutationPinsRegistryEntryUntilCompletion(t *testing.T) {
	fixture := newSettingsSpeakerFixture(t, true)
	...
	select {
	case <-removeDone:
		t.Fatal("registry entry changed while settings mutation was pinned")
	case <-time.After(50 * time.Millisecond):
	}

	close(fixture.infoRelease)
	...
	case removed := <-removeDone:
		if !removed {
			t.Fatal("pinned registry entry was not removed after mutation completion")
		}
	case <-time.After(2 * time.Second):
		t.Fatal("registry replacement remained blocked after mutation completion")
	}
	...
	if fixture.timeoutPosts.Load() != 1 {
		t.Fatalf("automatic-standby mutations = %d, want 1", fixture.timeoutPosts.Load())
	}
}
