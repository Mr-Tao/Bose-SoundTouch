...
func TestDeviceViewSnapshotConcurrentLastSeen(t *testing.T) {
	app := NewWebApp()
	...
	go func() {
		...
		for range operations {
			if _, err := json.Marshal(app.deviceViewSnapshot()); err != nil {
				t.Errorf("marshal device view snapshot: %v", err)
				return
			}
		}
	}()
