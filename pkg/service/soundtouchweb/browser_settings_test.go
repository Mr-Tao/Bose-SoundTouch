...
func TestSettingsOwnershipAndMutationStates(t *testing.T) {
	...
	if err := chromedp.Run(ctx,
		...
		chromedp.Evaluate(`(async () => {
            ...
            const snapshot = (label, enabled = false) => ({
            ...
            try {
                ...
                await original.clearBluetoothPairings('route-target');
                ...
                await original.setSystemTimeout('route-target', false);
                window.fetch = original.fetch;
                ...
                api.setClockDisplay = (deviceId, body) => {
                    const request = deferred();
                    mutations.push({ deviceId, body, ...request });
                    return request.promise;
                };
                ...
                loads[0].resolve({ success: true, data: snapshot('stale load data') });
                ...
                mutations[1].resolve({ success: true, data: snapshot('verified mutation', true) });
                ...
                api.clearBluetoothPairings = async deviceId => {
                    clearCalls += 1;
                    return {
                        success: false,
                        outcome: 'unverified',
	...
	if err := chromedp.Run(ctx,
		...
		chromedp.Evaluate(`(async () => {
            ...
            try {
                ...
                api.setSystemTimeout = async (deviceId, enabled) => {
                    calls.push({ deviceId, enabled });
                ...
                if (calls[0].deviceId !== 'standby-target' || calls[0].enabled !== false) {
